...
Make minimal changes to improve the match. Keep the overall structure but adjust the specific issues listed.`;
  ...
  /**
   * Generate pattern without LLM (fallback)
   */
  generateBasicPattern(analysis) {
    const { tempo, key, rhythm } = analysis;
    const cps = tempo / 60 / 4; // Convert BPM to cycles per second
    
    let pattern = `// Auto-generated pattern based on analysis\n`;
    pattern += `setcps(${cps.toFixed(3)})\n\n`;
    
    // Generate drum pattern
    if (rhythm.kick.length > 0) {
      const kickPattern = this.rhythmToEuclidean(rhythm.kick);
      pattern += `let drums = s("bd*${kickPattern}")\n`;
    } else {
      pattern += `let drums = s("bd*4")\n`;
    }
    
    if (rhythm.snare.length > 0) {
      pattern += `  .stack(s("sd*8").euclidLegato(${rhythm.snare.length}, 16))\n`;
    }
    
    if (rhythm.hihat.length > 0) {
      pattern += `  .stack(s("hh*16").gain(0.5))\n`;
    }
    
    // Add a simple bassline
    const rootNote = this.keyToMidi(key) - 24; // Two octaves down
    pattern += `\nlet bass = n("${rootNote} ${rootNote+7} ${rootNote+3} ${rootNote+5}")\n`;
    pattern += `  .s("bass").slow(2)\n`;
    
    pattern += `\nstack(drums, bass).room(0.2)`;
    
    return pattern;
  }

  /**
   * Convert rhythm positions to euclidean pattern
   */
  rhythmToEuclidean(positions) {
    if (positions.length === 0) return "4";
    
    // Simplified - just count hits in first beat
    const hitsInBeat = positions.filter(p => p < 1).length;
    return hitsInBeat > 0 ? hitsInBeat * 4 : 4;
  }
